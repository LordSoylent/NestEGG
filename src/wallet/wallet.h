...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 EGG at the same time
...
enum ZerocoinSpendStatus {
    ZEGG_SPEND_OKAY = 0,                            // No error
    ZEGG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZEGG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZEGG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZEGG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZEGG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZEGG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZEGG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZEGG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZEGG_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZEGG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZEGG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZEGG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZEGG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZEGG_SPENT_USED_ZEGG = 14,                      // Coin has already been spend
    ZEGG_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZEGG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZEggBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZEGGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzeggAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzEGGTracker> zeggTracker;
    ...
    void setZWallet(CzEGGWallet* zwallet);
    CzEGGWallet* getZWallet();
    bool isZeromintEnabled();
    void setZEggAutoBackups(bool fEnabled);
    ...
    /// Get 10000 EGG output and keys which can be used for the Masternode
    ...
    /** zEGG reset */
    boost::signals2::signal<void()> NotifyzEGGReset;
