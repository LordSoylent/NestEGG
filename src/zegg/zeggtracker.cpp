// Copyright (c) 2018-2020 The NESTEGG developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zegg/deterministicmint.h>
#include "zeggtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzEGGTracker::CzEGGTracker(std::string strWalletFile)
{
...
CzEGGTracker::~CzEGGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzEGGTracker::Init()
{
...
bool CzEGGTracker::Archive(CMintMeta& meta)
{
...
bool CzEGGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzEGGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzEGGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzEGGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzEGGTracker::GetSerialHashes()
{
...
CAmount CzEGGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzEGGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzEGGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzEGGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzEGGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzEGGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzEGGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzEGGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzEGGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzEGGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzEGGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzEGGWallet* zEGGWallet)
{
    bool iszEGGWalletInitialized = (NULL != zEGGWallet);
    CMintMeta meta;
    ...
    if (! iszEGGWalletInitialized)
        zEGGWallet = new CzEGGWallet(strWalletFile);
    meta.isSeedCorrect = zEGGWallet->CheckSeed(dMint);
    if (! iszEGGWalletInitialized)
        delete zEGGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzEGGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEGGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzEGGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzEGGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzEGGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzEGGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzEGGWallet* zEGGWallet = new CzEGGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zEGGWallet);
        }
        delete zEGGWallet;
        LogPrint("zero", "%s: added %d dzegg from DB\n", __func__, listDeterministicDB.size());
    }
