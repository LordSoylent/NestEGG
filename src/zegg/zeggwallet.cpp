...
CzEGGWallet::CzEGGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zegg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEGGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEGGSeed_deprecated()) {
                    LogPrintf("%s: Updated zEGG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zegg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zegg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEGGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEGGCount(nCountLastUsed);
    else if (!walletdb.ReadZEGGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEGGWallet::Lock()
{
    seedMaster = 0;
}

void CzEGGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEGGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEGG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEGGWallet::LoadMintPoolFromDB()
{
...
void CzEGGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEGGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEGGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zeggTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEGGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEGG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zeggTracker which also adds to database
    pwalletMain->zeggTracker->Add(dMint, true);
...
void CzEGGWallet::SeedToZEGG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEGGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEGGWallet::GenerateDeterministicZEGG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEGGWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEGGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzEGGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
