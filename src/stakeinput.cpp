...
CZEggStake::CZEggStake(const libzerocoin::CoinSpend& spend)
{
...
int CZEggStake::GetChecksumHeightFromMint()
{
...
int CZEggStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZEggStake::GetChecksum()
{
    return nChecksum;
}

// The zEGG block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZEggStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZEggStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zEGG staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZEggStake::GetUniqueness()
{
    //The unique identifier for a zEGG is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZEggStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZEggStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zEGG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZEGGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zEGG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zEGG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZEGGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zEGG output", __func__);
...
bool CZEggStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZEggStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzEGGTracker* zeggTracker = pwallet->zeggTracker.get();
    CMintMeta meta;
    if (!zeggTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zeggTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!EGG Stake
bool CEggStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CEggStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CEggStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CEggStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CEggStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CEggStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CEggStake::GetUniqueness()
{
...
CBlockIndex* CEggStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
