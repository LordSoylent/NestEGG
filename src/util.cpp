...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "nestegg" is a composite category enabling all NESTEGG-related debug output
            if (ptrCategory->count(std::string("nestegg"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "nestegg.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "nesteggd.pid"));
