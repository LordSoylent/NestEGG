...
/**
 * NESTEGG RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZeggSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzeggseed(const UniValue& params, bool fHelp);
extern UniValue getzeggseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzegg(const UniValue& params, bool fHelp);
extern UniValue dzeggstate(const UniValue& params, bool fHelp);
