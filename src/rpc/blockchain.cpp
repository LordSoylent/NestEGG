...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zeggObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zeggObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zeggObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zEGGsupply", zeggObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZEGG()) ? "Not available" : std::to_string(
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZEGG_FAILED_ACCUMULATOR_INITIALIZATION);
