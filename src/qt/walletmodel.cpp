...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 EGG.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZeggSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZegg(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzEGGReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzEGGReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzEGGReset.connect(boost::bind(NotifyzEGGReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzEGGReset.disconnect(boost::bind(NotifyzEGGReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zeggTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
