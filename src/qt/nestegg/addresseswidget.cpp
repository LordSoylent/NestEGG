...
#include "qt/nestegg/addresseswidget.h"
#include "qt/nestegg/forms/ui_addresseswidget.h"
#include "qt/nestegg/addresslabelrow.h"
#include "qt/nestegg/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(NESTEGGGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter NESTEGG address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress eggAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(eggAdd.Get(), label.toUtf8().constData(),
                eggAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
