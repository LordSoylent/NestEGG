...
#include "qt/nestegg/privacywidget.h"
#include "qt/nestegg/forms/ui_privacywidget.h"
#include "qt/nestegg/qtutils.h"
#include "guiutil.h"
#include "qt/nestegg/denomgenerationdialog.h"
#include "qt/nestegg/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(NESTEGGGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zEGG anonymizes your EGG by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zEGG or convert back to EGG"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 EGG ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zEGG");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zEGG");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zEGG");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zEGG");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zEGG");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zEGG");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zEGG");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zEGG");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zEGG Movements"));
    ...
    // Only Convert to EGG enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzEGG->setTitleClassAndText("btn-title-grey", "Total 0 zEGG");
    ui->btnTotalzEGG->setSubTitleClassAndText("text-subtitle", "Show denominations of zEGG owned.");
    ui->btnTotalzEGG->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select EGG outputs to mint into zEGG.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zEGG");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzEGG, SIGNAL(clicked()), this, SLOT(onTotalZeggClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zEGG");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of EGG to mint into zEGG"));
    }else{
        btnText = tr("Convert back to EGG");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zEGG to convert back into EGG"));
    }
...
void PrivacyWidget::onTotalZeggClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzEGG->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzEGG->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zEGG is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zEGG, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zEGG").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zEGG minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZegg(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zEGG converted back to EGG"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEGG </b>";
    ...
    ui->btnTotalzEGG->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
