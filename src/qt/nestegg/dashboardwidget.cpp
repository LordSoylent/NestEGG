...
#include "qt/nestegg/dashboardwidget.h"
#include "qt/nestegg/forms/ui_dashboardwidget.h"
#include "qt/nestegg/sendconfirmdialog.h"
#include "qt/nestegg/txrow.h"
...
DashboardWidget::DashboardWidget(NESTEGGGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of EGG and zEGG staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareEgg, "square-chart-egg");
    setCssProperty(ui->labelSquarezEgg, "square-chart-zegg");
    setCssProperty(ui->labelEgg, "text-chart-egg");
    setCssProperty(ui->labelZegg, "text-chart-zegg");
    ...
    ui->labelAmountZegg->setText("0 zEGG");
    ui->labelAmountEgg->setText("0 EGG");
    setCssProperty(ui->labelAmountEgg, "text-stake-egg-disable");
    setCssProperty(ui->labelAmountZegg, "text-stake-zegg-disable");
    ...
    ui->btnHowTo->setText(tr("How to get EGG or zEGG"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isEgg = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZEGG;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            egg = (pair.first != 0) ? pair.first / 100000000 : 0;
            zegg = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalEgg += pair.first;
            chartData->totalZegg += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesEgg.append(egg);
        chartData->valueszEgg.append(zegg);

        int max = std::max(egg, zegg);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("EGG");
    set1 = new QBarSet("zEGG");
    ...
    set0->append(chartData->valuesEgg);
    set1->append(chartData->valueszEgg);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalEgg > 0 || chartData->totalZegg > 0) {
        setCssProperty(ui->labelAmountEgg, "text-stake-egg");
        setCssProperty(ui->labelAmountZegg, "text-stake-zegg");
    } else {
        setCssProperty(ui->labelAmountEgg, "text-stake-egg-disable");
        setCssProperty(ui->labelAmountZegg, "text-stake-zegg-disable");
    }
    forceUpdateStyle({ui->labelAmountEgg, ui->labelAmountZegg});
    ui->labelAmountEgg->setText(GUIUtil::formatBalance(chartData->totalEgg, nDisplayUnit));
    ui->labelAmountZegg->setText(GUIUtil::formatBalance(chartData->totalZegg, nDisplayUnit, true));
