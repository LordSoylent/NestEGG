...
#include "qt/nestegg/send.h"
#include "qt/nestegg/forms/ui_send.h"
#include "qt/nestegg/addnewcontactdialog.h"
#include "qt/nestegg/qtutils.h"
#include "qt/nestegg/sendchangeaddressdialog.h"
#include "qt/nestegg/optionbutton.h"
#include "qt/nestegg/sendconfirmdialog.h"
#include "qt/nestegg/myaddressrow.h"
#include "qt/nestegg/guitransactionsutils.h"
...
SendWidget::SendWidget(NESTEGGGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("EGG");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zEGG");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (EGG) or private coins (zEGG)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("NESTEGG address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 EGG");
    ...
    setCssProperty(coinIcon, "coin-icon-egg");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onEGGSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onEGGSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZegg = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZegg));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZegg ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendEgg = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendEgg ? AskPassphraseDialog::Context::Send_EGG : AskPassphraseDialog::Context::Send_zEGG, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendEgg) ? send(recipients) : sendZegg(recipients)) {
...
bool SendWidget::sendZegg(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zEGG is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zEGG selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZEggControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZEggControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZegg(
        ...
        inform(tr("zEGG transaction sent!"));
        ZEggControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZEGG_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zEGG require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zEGG transaction
        ...
        emit message("zEGG transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isEGG){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any EGG to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZEggControlDialog *zEggControl = new ZEggControlDialog(this);
            zEggControl->setModel(walletModel);
            zEggControl->exec();
            ui->btnCoinControl->setActive(!ZEggControlDialog::setSelectedMints.empty());
            zEggControl->deleteLater();
        } else {
            inform(tr("You don't have any zEGG in your balance to select."));
        }
    }
}
...
void SendWidget::onEGGSelected(bool _isEGG){
    isEGG = _isEGG;
    setCssProperty(coinIcon, _isEGG ? "coin-icon-egg" : "coin-icon-zegg");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress eggAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(eggAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(eggAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
