...
#include "qt/nestegg/splash.h"
#include "qt/nestegg/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("nestegg-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in nestegg.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in nestegg.qrc)
...
/** Class encapsulating NESTEGG Core startup and shutdown.
...
/** Main NESTEGG application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (NESTEGGGUI)
...
{
    window = new NESTEGGGUI(networkStyle, 0);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(NESTEGGGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(NESTEGGGUI::DEFAULT_WALLET);
        ...
        // NESTEGG: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", NESTEGGGUI::tr("A fatal error occurred. NESTEGG can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(nestegg_locale);
    Q_INIT_RESOURCE(nestegg);
