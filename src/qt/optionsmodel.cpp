...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::EGG);
    ...
    if (settings.contains("nAnonymizeNestEggAmount") || reset)
        SoftSetArg("-anonymizenesteggamount", settings.value("nAnonymizeNestEggAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nestegg.cpp during shotdown to also remove the window positions
