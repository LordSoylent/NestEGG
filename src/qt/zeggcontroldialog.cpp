...
#include "ui_zeggcontroldialog.h"
...
std::set<std::string> ZEggControlDialog::setSelectedMints;
std::set<CMintMeta> ZEggControlDialog::setMints;

bool CZEggControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZEggControlDialog::COLUMN_DENOMINATION || column == ZEggControlDialog::COLUMN_VERSION || column == ZEggControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZEggControlDialog::ZEggControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZEggControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zEGG Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZEgg->setProperty("cssClass", "text-main-purple");
    ui->labelZEgg_int->setProperty("cssClass", "text-main-purple");
...
ZEggControlDialog::~ZEggControlDialog()
{
    delete ui;
}

void ZEggControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEggControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZEggControlWidgetItem* itemDenom(new CZEggControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZEggControlWidgetItem *itemMint = new CZEggControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zEGG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zEGG seed used to mint this zEGG is not the same as currently hold in the wallet";
            else
...
void ZEggControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEggControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZEgg_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZEggControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZEggControlDialog::GetSelectedMints()
{
...
void ZEggControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
