...
class ZerocoinSpendTest(NestEggTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zegg_bal, egg_bal):
            zegg_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zegg_bal)
            egg_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], egg_bal)
            return zegg_bal, egg_bal
        ...
        zegg_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zegg_balance, 6666)
        ...
        zegg_balance, balance = check_balances(denom_0, zegg_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zegg_balance, balance = check_balances(denom_2, zegg_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zegg_balance, balance = check_balances(denom_3, zegg_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zegg",
        ...
        _, _ = check_balances(denom_1, zegg_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
